...
import {
  Package, ArrowRight, PlusCircle, FileText, Download,
  CheckCircle2, BarChart3, Users, AlertTriangle, Warehouse,
} from 'lucide-react';
...
export function Dashboard() {
  ...
  const quickActions = [
    { label: 'Nouveau produit', icon: PlusCircle, color: 'text-blue-600' },
    { label: 'Bon de commande', icon: FileText, color: 'text-indigo-600' },
    { label: 'Entrée stock', icon: Download, color: 'text-emerald-600' },
    { label: 'Valider commande', icon: CheckCircle2, color: 'text-purple-600' },
    { label: 'Voir rapports', icon: BarChart3, color: 'text-orange-600' },
    { label: 'Utilisateurs', icon: Users, color: 'text-pink-600' },
  ];

  const statCards = data
    ? [
        { title: 'Produits', value: data.statistiques.total_produits, icon: Package, color: 'red' as const },
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        {loading
          ...
          : statCards.map((stat, index) => (
              <StatCard
                ...
                title={stat.title}
                value={stat.value}
                icon={<stat.icon className="w-15 h-15 " />}
                color={stat.color}
              />
            ))}
      </div>
          ...
          <Card className="border-gray-100 shadow-sm bg-gray-50/50">
            ...
            <CardContent className="grid grid-cols-2 gap-3">
              {quickActions.map((item, index) => (
                <button
                  key={index}
                  className="flex flex-col items-center justify-center p-4 bg-white rounded-md border border-gray-400 hover:border-royal-500 hover:bg-royal-50 transition-all active:scale-95"
                >
      ...
      <div className="text-center text-xs text-gray-900 pt-6 border-t border-gray-700">
