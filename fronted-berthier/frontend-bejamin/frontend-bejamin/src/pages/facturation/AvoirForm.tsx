          ...
          <CardContent className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select value={values.id_facture_origine} onValueChange={(v) => set('id_facture_origine', v)}>
                  <SelectTrigger className={cn('w-full h-11 border-gray-200 shadow-sm', errorClass(fieldErrors.id_facture_origine))}>
                    <SelectValue placeholder="Optionnelle" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value=" ">Aucune</SelectItem>
                    {factures.map((f) => (<SelectItem key={f.id} value={String(f.id)}>{f.numero_facture}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
              <div>
