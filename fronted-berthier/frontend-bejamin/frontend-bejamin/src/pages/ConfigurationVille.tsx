...
import type { Ville } from '../types/ville';
import {
  Plus, Search, Pencil, Trash2, RefreshCw, Globe, CheckCircle, XCircle,
} from 'lucide-react';
import { cn } from '../lib/utils';

export function ConfigurationVille() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<Ville | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await villeService.list(params);
      if (res.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      if (editing) {
        await villeService.update(editing.id, form);
        toast('Ville modifiée avec succès', 'success');
      } else {
        await villeService.create(form as any);
        toast('Ville créée avec succès', 'success');
      }
      setPanelOpen(false);
      fetchData();
    } catch (err: unknown) {
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await villeService.delete(deleteTarget.id);
      toast('Ville supprimée avec succès', 'success');
    ...
    <div className="space-y-6">
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg font-semibold">Liste des villes</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-lg font-medium text-gray-700">Aucune ville trouvée</p>
              <p className="text-sm mt-1">Commencez par créer une nouvelle ville</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto rounded-lg border border-gray-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <div>
                <span className="text-sm font-medium text-white">Ville active</span>
