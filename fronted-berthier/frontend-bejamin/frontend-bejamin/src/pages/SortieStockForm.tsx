...
export function SortieStockForm() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const params: Record<string, string> = { per_page: String(pageSize), page: String(currentPage), sort_by: 'date_mouvement', sort_order: 'desc' };
      if (searchTerm) params.search = searchTerm;
      const res = await mouvementStockService.list(params);
      if (res.success) {
        setData(res.data.data.filter((m: MouvementStock) => m.type_mouvement?.sens === -1));
