...
import {
  Plus, Search, Pencil, Trash2, RefreshCw, MapPin, Globe, CheckCircle, XCircle,
} from 'lucide-react';
...
export function ConfigurationZone() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (typeFilter && typeFilter !== '_all') params.type_zone = typeFilter;

      const res = await zoneService.list(params);
      if (res.success) {
  ...
  }, [currentPage, searchTerm, villeFilter]);
  ...
  useEffect(() => {
    zoneService.getVilles({ per_page: '200', sort_by: 'nom', sort_order: 'asc' })
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ nom: '', id_ville: '', type_zone: 'stockage', description: '', actif: true });
    setFieldErrors({});
    setPanelOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      if (editing) {
        await zoneService.update(editing.id, form);
        toast('Zone modifiée avec succès', 'success');
      } else {
        await zoneService.create(form as any);
        toast('Zone créée avec succès', 'success');
      }
      setPanelOpen(false);
      fetchData();
    } catch (err: unknown) {
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await zoneService.delete(deleteTarget.id);
      toast('Zone supprimée avec succès', 'success');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {loading ? '...' : `${total} zone${total > 1 ? 's' : ''}`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => { setSearchInput(''); setSearchTerm(''); setVilleFilter('_all'); setTypeFilter('_all'); setCurrentPage(1); }} className="border-gray-300 text-gray-700 hover:bg-gray-50" title="Actualiser">
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg font-semibold">Liste des zones</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-lg font-medium text-gray-700">Aucune zone trouvée</p>
              <p className="text-sm mt-1">Commencez par créer une nouvelle zone</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <Table>
                  ...
                  <TableBody>
                    {data.map((z, i) => (
                      <TableRow key={z.id} className={cn('hover:bg-royal-50/50 transition-colors', i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50')}>
                        <TableCell className="font-medium text-gray-900">{z.nom}</TableCell>
                        <TableCell>
                          <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-medium">
                            {typeLabel[z.type_zone] || z.type_zone}
                          </span>
                        </TableCell>
                        <TableCell className="hidden md:table-cell text-sm text-gray-600">{z.ville?.nom || '-'}</TableCell>
                        <TableCell className="hidden lg:table-cell text-sm text-gray-600 max-w-[200px] truncate">{z.description || '-'}</TableCell>
                        <TableCell className="text-center">
                          <span className={cn(
                            'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium',
                            z.actif ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-600',
                          )}>
                            {z.actif ? <><CheckCircle className="w-3 h-3 mr-1" /> Actif</> : <><XCircle className="w-3 h-3 mr-1" /> Inactif</>}
                          </span>
                        </TableCell>
                        <TableCell className="text-center">
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => setDeleteTarget(z)}
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <Label className="text-sm font-medium text-white/80">Type de zone</Label>
            <Select value={form.type_zone} onValueChange={(v) => setForm((f) => ({ ...f, type_zone: v }))}>
              <SelectTrigger className="mt-1.5 bg-royal-700 border-royal-600 text-white focus:border-royal-500 focus:ring-royal-500 h-11">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div>
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <div>
                <span className="text-sm font-medium text-white">Zone active</span>
