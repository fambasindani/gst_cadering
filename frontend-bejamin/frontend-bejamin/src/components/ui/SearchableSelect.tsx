...
export function SearchableSelect({
  ...
  const updatePosition = useCallback(() => {
    ...
    const estimatedDropdownHeight = Math.min(filtered.length * 40 + 60, 240);
    if (spaceBelow < estimatedDropdownHeight && spaceAbove > spaceBelow) {
      setDropdownUp(true);
      setMaxHeight(Math.min(spaceAbove - 16, 240));
    } else {
      setDropdownUp(false);
      setMaxHeight(Math.min(spaceBelow - 16, 240));
    }
  }, [filtered.length]);
    ...
    <div ref={containerRef} className={cn('relative', className)}>
      ...
      {open && createPortal(
        <div id={instanceId.current}
          style={{
            position: 'fixed',
            top: dropdownUp ? pos.top - maxHeight - 4 : pos.bottom + 4,
            left: pos.left,
            width: pos.width,
            zIndex: 99999,
          }}
