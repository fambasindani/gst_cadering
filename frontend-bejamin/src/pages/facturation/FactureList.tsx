...
export function FactureList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await factureService.list(params);
      if (res.success) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await factureService.delete(deleteTarget.id);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{loading ? '...' : `${total} facture${total > 1 ? 's' : ''}`}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => navigate('/facturation/factures/creer')} className="bg-royal-700 hover:bg-royal-800 text-white shadow-sm">
        ...
        <CardContent>
          {loading ? (
              ...
              <Table>
                <TableHeader className="bg-gray-50">
                  <TableRow>
                    ...
                    <TableHead className="text-right font-semibold text-gray-600">Montant TTC</TableHead>
                    <TableHead className="text-right font-semibold text-gray-600">Solde</TableHead>
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-lg font-medium text-gray-700">Aucune facture</p>
              <p className="text-sm mt-1">Cliquez sur "Nouvelle facture" pour créer une facture</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <Table>
                  ...
                  <TableBody>
                    {data.map((f, i) => (
                      <TableRow key={f.id} className={cn('hover:bg-royal-50/50 transition-colors cursor-pointer', i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50')} onClick={() => navigate(`/facturation/factures/${f.id}`)}>
                        <TableCell className="font-mono text-sm font-medium text-royal-700">{f.numero_facture}</TableCell>
                        <TableCell className="font-medium text-gray-900">{f.client?.nom || '-'}</TableCell>
                        <TableCell className="text-sm text-gray-600">{formatDate(f.date_facture)}</TableCell>
                        <TableCell className="text-right font-mono text-sm font-medium text-gray-900">{formatCurrency(f.montant_ttc, f.devise?.code)}</TableCell>
                        <TableCell className={cn('text-right font-mono text-sm font-medium', (f.solde || 0) > 0 ? 'text-red-600' : 'text-green-600')}>
                          {formatCurrency(f.solde ?? f.montant_ttc, f.devise?.code)}
                        </TableCell>
                        <TableCell className="text-center">
                          <Badge variant={statutVariants[f.statut] || 'secondary'} className="text-xs whitespace-nowrap">
                            {statutLabels[f.statut] || f.statut}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center justify-center gap-1" onClick={(e) => e.stopPropagation()}>
                            <button onClick={() => navigate(`/facturation/factures/${f.id}`)} className="p-1.5 rounded text-gray-500 hover:text-blue-700 hover:bg-blue-50 transition-colors" title="Voir">
                              <Eye className="w-4 h-4" />
                            </button>
                            {f.statut === 'BROUILLON' && (
                              <button onClick={() => navigate(`/facturation/factures/${f.id}/modifier`)} className="p-1.5 rounded text-gray-500 hover:text-royal-700 hover:bg-royal-50 transition-colors" title="Modifier">
                                <Pencil className="w-4 h-4" />
                              </button>
                            )}
                            {f.statut === 'BROUILLON' && (
                              <button onClick={() => setDeleteTarget(f)} className="p-1.5 rounded text-gray-500 hover:text-red-700 hover:bg-red-50 transition-colors" title="Supprimer">
      ...
      <ConfirmModal
        ...
        message={`Supprimer la facture "${deleteTarget?.numero_facture}" ? Cette action est irréversible.`}
        variant="danger"
