...
export function FactureForm() {
  ...
  const [lots, setLots] = useState<{ id: number; numero_lot: string; id_produit: number; quantite_disponible: number; produit?: { nom: string } | null }[]>([]);
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      lotService.list({ per_page: '500', statut: 'VALIDÉ' }),
    ]).then(([c, v, d, p, b, l]) => {
      ...
      if (l.status === 'fulfilled' && l.value.success) setLots(l.value.data.data);
      const rejected = [c, v, d, p, b, l].filter(r => r.status === 'rejected');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && id) {
        await factureService.update(Number(id), payload as never);
...
<SearchableSelect
                        ...
                        <TableCell className="min-w-[180px]">
                          <SearchableSelect
                            options={lots
                              .filter(l => !r.id_produit || l.id_produit === Number(r.id_produit))
                              .map(l => ({ id: l.id, nom: l.numero_lot, sousTitre: `dispo: ${l.quantite_disponible}${l.produit?.nom ? ` - ${l.produit.nom}` : ''}` }))}
                            value={r.id_lot}
                            onValueChange={(v) => updateLigne(r.key, 'id_lot', v)}
