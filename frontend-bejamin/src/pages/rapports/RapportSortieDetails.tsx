...
import {
  ArrowLeft, Package, Building2, MapPin, Calendar, Hash,
  FileText, User, Clock, CheckCircle, DollarSign,
} from 'lucide-react';
import { cn } from '../../lib/utils';
import { formatCurrency } from '../../lib/format';

export function RapportSortieDetails() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (res.success) setMouvement(res.data);
    } catch {
      //
    } finally {
      setLoading(false);
  ...
  if (!mouvement) {
    return (
      <div className="text-center py-12">
        ...
        <Button variant="outline" className="mt-4" onClick={() => navigate('/rapports/sortie')}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" onClick={() => navigate('/rapports/sortie')} className="p-0 h-9 w-9 text-gray-500 hover:text-gray-700">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <SummaryCard
          ...
          label="Date"
          value={mouvement.date_mouvement ? new Date(mouvement.date_mouvement).toLocaleDateString('fr-FR') : '-'}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-0 shadow-sm">
            ...
            <CardContent className="p-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <InfoRow icon={<Calendar className="w-4 h-4 text-gray-500" />} label="Date" value={mouvement.date_mouvement ? new Date(mouvement.date_mouvement).toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : '-'} />
        ...
        <div className="space-y-6">
          <Card className="border-0 shadow-sm">
            ...
            <CardContent className="p-4 space-y-3">
              <SummaryMini label="Client" value={mouvement.partenaire?.nom || '-'} icon={<Building2 className="w-4 h-4 text-royal-600" />} />
              <SummaryMini label="Département" value={mouvement.departement?.nom || '-'} icon={<Building2 className="w-4 h-4 text-gray-500" />} />
              <SummaryMini label="Magasin" value={lot?.magasin?.nom || '-'} icon={<MapPin className="w-4 h-4 text-gray-500" />} />
              <SummaryMini label="Fournisseur" value={lot?.partenaire?.nom || '-'} icon={<Building2 className="w-4 h-4 text-gray-500" />} />
              ...
              {mouvement.date_validation && (
                <SummaryMini label="Date validation" value={new Date(mouvement.date_validation).toLocaleDateString('fr-FR')} icon={<Clock className="w-4 h-4 text-gray-500" />} />
              )}
            </CardContent>
          </Card>

          <Button variant="outline" className="w-full justify-start border-gray-200 text-gray-700 hover:bg-gray-50"
            onClick={() => navigate('/rapports/sortie')}>
