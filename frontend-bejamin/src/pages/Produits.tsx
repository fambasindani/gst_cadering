...
import {
  Plus, Search, Pencil, Trash2, Eye, RefreshCw, Package, CheckCircle, XCircle,
} from 'lucide-react';
...
export function Produits() {
  ...
  const [categorieFilter, setCategorieFilter] = useState('_all');
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (categorieFilter && categorieFilter !== '_all') params.categorie_id = categorieFilter;
  ...
  }, [currentPage, searchTerm, categorieFilter]);
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await produitService.delete(deleteTarget.id);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => { setSearchInput(''); setSearchTerm(''); setCategorieFilter('_all'); setCurrentPage(1); }} className="border-gray-300 text-gray-700 hover:bg-gray-50" title="Actualiser">
            <RefreshCw className={cn('h-4 w-4 mr-2', loading && 'animate-spin')} />
            Actualiser
          </Button>
          <Button onClick={() => navigate('/produits/creer')} className="bg-royal-700 hover:bg-royal-800 text-white shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <Select value={categorieFilter} onValueChange={(v) => { setCategorieFilter(v); setCurrentPage(1); }}>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg font-semibold">Liste des produits</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
              ...
              <Table>
                <TableHeader className="bg-gray-50">
                  <TableRow>
                    ...
                    <TableHead className="hidden md:table-cell font-semibold text-gray-600">Catégorie</TableHead>
                    <TableHead className="hidden lg:table-cell font-semibold text-gray-600">Unité</TableHead>
                ...
                <TableBody>
                  {Array.from({ length: 5 }).map((_, i) => (
                    <TableRow key={i} className="animate-pulse">
                      ...
                      <TableCell className="hidden lg:table-cell"><div className="h-5 w-16 bg-gray-200 rounded" /></TableCell>
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-sm mt-1">Commencez par créer un nouveau produit</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                  ...
                  <TableBody>
                    {data.map((p, i) => (
                      <TableRow key={p.id} className={cn('hover:bg-royal-50/50 transition-colors', i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50')}>
                        <TableCell className="font-mono text-sm font-medium text-gray-900">{p.code_article}</TableCell>
                        <TableCell className="font-medium text-gray-900">{p.nom}</TableCell>
                        <TableCell className="hidden md:table-cell text-sm text-gray-600">{p.categorie?.nom || '-'}</TableCell>
                        <TableCell className="hidden lg:table-cell text-sm text-gray-600">{p.unite?.symbole || '-'}</TableCell>
                        <TableCell className="text-center">
                          <span className={cn(
                            'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium',
                            p.actif ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-600',
                          )}>
                            {p.actif ? <><CheckCircle className="w-3 h-3 mr-1" /> Actif</> : <><XCircle className="w-3 h-3 mr-1" /> Inactif</>}
                          </span>
                        </TableCell>
                        <TableCell className="text-center">
                          <div className="flex items-center justify-center gap-1">
                            <Button variant="ghost" size="sm" onClick={() => navigate(`/produits/${p.id}`)}
                              className="h-8 w-8 p-0 text-gray-600 hover:text-gray-700 hover:bg-gray-100 rounded-lg" title="Détails">
                              <Eye className="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => navigate(`/produits/${p.id}/modifier`)}
