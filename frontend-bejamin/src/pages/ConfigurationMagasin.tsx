...
import type { Magasin } from '../types/magasin';
import {
  Plus, Search, Pencil, Trash2, RefreshCw, Globe, CheckCircle, XCircle,
} from 'lucide-react';
import { cn } from '../lib/utils';

export function ConfigurationMagasin() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await magasinService.list(params);
      if (res.success) {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ nom: '', code: '', pays: 'RDC', actif: true });
    setFieldErrors({});
    setPanelOpen(true);
  };

  const openEdit = (magasin: Magasin) => {
    setEditing(magasin);
    setForm({
      nom: magasin.nom,
      code: magasin.code || '',
      pays: magasin.pays || 'RDC',
      actif: magasin.actif,
    });
    setFieldErrors({});
    setPanelOpen(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      if (editing) {
        await magasinService.update(editing.id, form);
        toast('Magasin modifiée avec succès', 'success');
      } else {
        await magasinService.create(form as any);
        toast('Magasin créée avec succès', 'success');
      }
      setPanelOpen(false);
      fetchData();
    } catch (err: unknown) {
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await magasinService.delete(deleteTarget.id);
      toast('Magasin supprimée avec succès', 'success');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {loading ? '...' : `${total} magasin${total > 1 ? 's' : ''}`}
          </p>
        </div>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg font-semibold">Liste des magasins</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
              ...
              <Table>
                <TableHeader className="bg-gray-50">
                  <TableRow>
                    ...
                    <TableHead className="hidden md:table-cell font-semibold text-gray-600">Pays</TableHead>
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-lg font-medium text-gray-700">Aucun magasin trouvé</p>
              <p className="text-sm mt-1">Commencez par créer une nouveau magasin</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <Table>
                  ...
                  <TableBody>
                    {data.map((v, i) => (
                      <TableRow key={v.id} className={cn('hover:bg-royal-50/50 transition-colors', i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50')}>
                        <TableCell className="font-medium text-gray-900">{v.nom}</TableCell>
                        <TableCell><span className="text-sm font-mono text-gray-600">{v.code || '-'}</span></TableCell>
                        <TableCell className="hidden md:table-cell text-sm text-gray-600">{v.pays || '-'}</TableCell>
                        <TableCell className="text-center">
                          <span className={cn(
                            'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium',
                            v.actif ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-600',
                          )}>
                            {v.actif ? <><CheckCircle className="w-3 h-3 mr-1" /> Actif</> : <><XCircle className="w-3 h-3 mr-1" /> Inactif</>}
                          </span>
                        </TableCell>
                        <TableCell className="text-center">
      ...
      <SlidePanel
        ...
        title={editing ? 'Modifier le magasin' : 'Nouveau magasin'}
        width="lg"
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <Label className="text-sm font-medium text-white/80">Pays</Label>
            <Input
              value={form.pays}
              onChange={(e) => { setForm((f) => ({ ...f, pays: e.target.value })); setFieldErrors((f) => ({ ...f, pays: '' })); }}
              placeholder="Pays"
              className={cn("mt-1.5 bg-royal-700 text-white placeholder:text-white/40 focus:border-royal-500 focus:ring-royal-500 h-11",
                fieldErrors.pays ? 'border-red-400' : 'border-royal-600')}
            />
            {fieldErrors.pays && <p className="text-red-400 text-xs mt-1">{fieldErrors.pays}</p>}
          </div>
          ...
          <div>
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <div>
                <span className="text-sm font-medium text-white">Magasin active</span>
