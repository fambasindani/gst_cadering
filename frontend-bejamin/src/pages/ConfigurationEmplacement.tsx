...
import { emplacementService } from '../services/emplacement';
import type { Emplacement } from '../types/emplacement';
import {
  Plus, Search, Pencil, Trash2, RefreshCw, MapPin, CheckCircle, XCircle,
} from 'lucide-react';
import { cn } from '../lib/utils';

export function ConfigurationEmplacement() {
  const { toast } = useToast();

  const [data, setData] = useState<Emplacement[]>([]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<Emplacement | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await emplacementService.list(params);
      if (res.success) {
  ...
  useEffect(() => {
    emplacementService.getZones({ per_page: '200', sort_by: 'nom', sort_order: 'asc' })
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ nom: '', id_zone: '', description: '', actif: true });
    setFieldErrors({});
    setPanelOpen(true);
  };
  ...
  const openEdit = (emplacement: Emplacement) => {
    ...
    setForm({
      nom: emplacement.nom,
      id_zone: String(emplacement.id_zone),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      if (editing) {
        await emplacementService.update(editing.id, form);
        toast('Emplacement modifié avec succès', 'success');
      } else {
        await emplacementService.create(form as any);
        toast('Emplacement créé avec succès', 'success');
      }
      setPanelOpen(false);
      fetchData();
    } catch (err: unknown) {
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await emplacementService.delete(deleteTarget.id);
      toast('Emplacement supprimé avec succès', 'success');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {loading ? '...' : `${total} emplacement${total > 1 ? 's' : ''}`}
          </p>
        </div>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg font-semibold">Liste des emplacements</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-500">
              ...
              <p className="text-lg font-medium text-gray-700">Aucun emplacement trouvé</p>
              <p className="text-sm mt-1">Commencez par créer un nouvel emplacement</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <Table>
                  ...
                  <TableBody>
                    {data.map((e, i) => (
                      <TableRow key={e.id} className={cn('hover:bg-royal-50/50 transition-colors', i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50')}>
                        <TableCell className="font-medium text-gray-900">{e.nom}</TableCell>
                        <TableCell className="text-sm text-gray-600">{e.zone?.nom || '-'}</TableCell>
                        <TableCell className="hidden lg:table-cell text-sm text-gray-600 max-w-[200px] truncate">{e.description || '-'}</TableCell>
                        <TableCell className="text-center">
                          <span className={cn(
                            'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium',
                            e.actif ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-600',
                          )}>
                            {e.actif ? <><CheckCircle className="w-3 h-3 mr-1" /> Actif</> : <><XCircle className="w-3 h-3 mr-1" /> Inactif</>}
                          </span>
                        </TableCell>
                        <TableCell className="text-center">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <Label className="text-sm font-medium text-white/80">Zone *</Label>
            <SearchableSelect
              options={zones.map(z => ({ id: z.id, nom: z.nom }))}
              value={form.id_zone}
              onValueChange={(v) => { setForm((f) => ({ ...f, id_zone: v })); setFieldErrors((f) => ({ ...f, id_zone: '' })); }}
          ...
          <div>
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <div>
                <span className="text-sm font-medium text-white">Emplacement actif</span>
